...
**Everything a submodule declares public is re-exported here.** That is the whole
policy, and it exists because the alternative was measured: with only part of the
surface re-exported, ``from tempest_core import Input`` raised ImportError and a
consumer had no choice but to reach into ``tempest_core.widgets.inputs`` — 50
files in tempestweb did exactly that, against their own house rule of importing
from the package root. A partial root is a root that teaches people to bypass it.

Each name is re-exported in **both** forms — ``from x import Y as Y`` *and* an
entry in :data:`__all__`. The redundancy is not style: without the ``as`` form,
basedpyright and Pylance in strict mode report "private import usage" at every
consumer call site, and ``__all__`` alone does not silence it.

``tests/test_public_surface.py`` fails if a submodule ever gains a public name
this module does not re-export, so the two can no longer drift.
"""
...
from tempest_core.icons import (
    ICON_PATHS as ICON_PATHS,
)
from tempest_core.icons import (
    MATERIAL_ALIASES as MATERIAL_ALIASES,
)
from tempest_core.icons import (
    Icons as Icons,
)
from tempest_core.icons import (
    icon_names as icon_names,
)
from tempest_core.icons import (
    icon_path as icon_path,
)
from tempest_core.icons import (
    register_icon as register_icon,
)
from tempest_core.icons import (
    svg_to_path as svg_to_path,
)
...
from tempest_core.style import (
    SideBorder as SideBorder,
)
from tempest_core.style import (
    Size as Size,
)
from tempest_core.style import (
    StackAlign as StackAlign,
)
from tempest_core.style import (
    Style as Style,
)
...
from tempest_core.validators import (
    validate_cnpj as validate_cnpj,
)
from tempest_core.validators import (
    validate_cpf as validate_cpf,
)
from tempest_core.validators import (
...
from tempest_core.variants import (
    DISABLED_CONTAINER_OPACITY as DISABLED_CONTAINER_OPACITY,
)
from tempest_core.variants import (
    DISABLED_CONTENT_OPACITY as DISABLED_CONTENT_OPACITY,
)
from tempest_core.variants import (
    ELEVATION_SHADOW_COLOR as ELEVATION_SHADOW_COLOR,
)
from tempest_core.variants import (
    FOCUS_OPACITY as FOCUS_OPACITY,
)
from tempest_core.variants import (
    HOVER_OPACITY as HOVER_OPACITY,
)
from tempest_core.variants import (
    MIN_TOUCH_TARGET as MIN_TOUCH_TARGET,
)
from tempest_core.variants import (
    PRESSED_OPACITY as PRESSED_OPACITY,
)
from tempest_core.variants import (
...
from tempest_core.variants import (
    VALID_COLOR_SCHEMES as VALID_COLOR_SCHEMES,
)
from tempest_core.variants import (
    ResponsiveSize as ResponsiveSize,
)
from tempest_core.variants import (
    merge_styles as merge_styles,
)
from tempest_core.variants import (
